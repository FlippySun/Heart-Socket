...
- **心率区间变色** — 编程场景优化的 9 级区间（深度放松/放松/平静/轻度集中/专注/紧张/高压/异常），在静息范围超细粒度划分
- **多数据源支持** — HDS 本地直连 / HypeRate / Pulsoid / 自定义 WebSocket
- **智能告警** — 高心率/低心率弹窗提醒，可配置阈值和冷却时间
- **自动重连** — 网络断开后指数退避自动重连
- **心率统计面板** — 实时仪表盘：趋势图（1分钟~12小时时间尺度）、区间分布饼图、9级图谱条、健康数据总览
- **📅 历史日历** — 日历月视图查看历史心率日报，持久化存储 90 天
- **网络变化检测** — WiFi/IP 变化时自动弹窗提醒，引导更新 Watch 地址
- **⌨️ 敲代码强度** — 基于手腕加速度实时分析打字强度（💤⌨️⚡🔥🚀 五级指示），状态栏常驻显示
- **🪑 久坐提醒** — 步数 + 加速度综合判断，超过阈值弹窗提醒（默认 45 分钟）
- **🖐️ 姿态感知** — 5 种姿态识别（😴静息/⌨️打字/🖱️鼠标/💪活动/🚶走动），离开工位超时弹框提醒（默认 30 秒）
...
> Motion 分析引擎基于 Apple Watch 手腕加速度传感器，采用 **VMUA (Vector Magnitude of User Acceleration)** 算法，对 HDS 发送的 `userAcceleration`（已去除重力）进行实时分析。

**算法特点：**
- 单 epoch = 1 秒，采用 3 秒滑动窗口平滑
- VMUA = √(ax² + ay² + az²)，去除重力后的纯用户加速度向量模
- 基于 VMUA 阈值划分 5 种姿态：
  - 😴 **静息** (VMUA < 0.003g) — 手腕完全静止
  - ⌨️ **打字中** (0.003g-0.015g) — 结合编辑器活动融合判定，支持左手打字检测
  - 🖱️ **鼠标操作** (0.003g-0.015g) — 无编辑器活动时的手腕微动
  - 💪 **活动中** (0.015g-0.08g) — 较大幅度手臂运动
  - 🚶 **走动** (> 0.08g) — 行走、离开工位
...
| `heartSocket.bodyMassJsonPath` | string | `""` | 自定义数据源体重字段 JSON 路径（留空不启用） |
| `heartSocket.bmiJsonPath` | string | `""` | 自定义数据源 BMI 字段 JSON 路径（留空不启用） |
| `heartSocket.statusBarPosition` | enum | `left` | 状态栏位置 |
| `heartSocket.showHeartbeatAnimation` | boolean | `true` | 心跳动画 |
| `heartSocket.zones` | object | `{deepRelax:58,relax:65,...}` | 心率区间阈值（9 级，编程场景优化） |
| `heartSocket.enableMotion` | boolean | `true` | 启用 Motion 传感器数据分析（需 HDS） |
| `heartSocket.sedentaryMinutes` | number | `45` | 久坐提醒阈值（分钟，10-120），提醒后自动进入冷却期，持续活动才重置计时 |
| `heartSocket.postureAlertSeconds` | number | `30` | 离开工位提醒阈值（秒，10-300）— 持续处于 active/walking 姿态时触发 |
...
> 针对开发者久坐编程场景，在 50-120 BPM 范围内做了 9 级细粒度划分，让你能精确感知编程时的心理状态变化。

| 区间 | BPM 范围 | 颜色 | 说明 |
|------|----------|------|------|
| ⚠️ 偏低 | < 50 | 🔵 蓝色 | 低于告警阈值 |
| 😪 深度放松 | 50-58 | 🟣 靛蓝 | 深度休息、冥想 |
| 😴 放松 | 58-65 | 🔵 蓝色 | 轻松浏览、摸鱼 |
| 😌 平静 | 65-72 | 🟢 绿色 | 阅读文档、Review 代码 |
| 🧘 轻度集中 | 72-80 | 🟢 青色 | 思考设计、写伪代码 |
| 🧠 专注 | 80-90 | 🟣 紫色 | 深度编码、解决问题 |
| 😰 紧张 | 90-105 | 🟡 黄色 | Debug、赶 deadline |
| 😤 高压 | 105-120 | 🟠 橙色 | 线上事故、紧急会议 |
...
├── types.ts              # 类型定义
├── config.ts             # 配置管理
├── dataStore.ts          # 数据持久化（日摘要，globalState，90 天保留）
├── webSocketClient.ts    # WebSocket 客户端（含自动重连）
├── motionAnalyzer.ts     # Motion v3 分析引擎（VMUA 算法，5 种姿态，久坐冷却）
...
│   └── customProvider.ts   # 自定义 WebSocket（多字段 JSON Path 提取，含 bodyMass/bmi）
├── statusBarManager.ts   # 状态栏 UI（心率 + 敲代码强度）
├── alertManager.ts       # 告警通知
└── heartRateManager.ts   # 核心管理器（协调 Motion 分析器 + Stats 面板 + 日历历史）
```
...
```
Apple Watch (HDS App)
    │ motion:{userAcceleration, gravity, rotationRate, attitude}
    ▼
...
heartRateManager.ts     ── 转发数据 + 协调各模块
    ...
    ▼
motionAnalyzer.ts       ── v3 VMUA 算法引擎（3s 窗口，1s 输出）
    │                          ├─ VMUA = √(ax²+ay²+az²) 姿态判定
    │                          ├─ 不活动 bout 久坐检测（含冷却机制）
    │                          └─ 编辑器活动融合（左手打字检测）
...
heartRateManager.ts     ── 弹窗告警（久坐/姿态/心流，含防重复弹窗保护）
```
